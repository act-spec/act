...
async function main(): Promise<void> {
  ...
  console.log(`docusaurus-docs conformance — ${nodeFiles.length} node files, ${subtreeFiles.length} subtree files.`);
  ...
  if (blockingWarnings.length > 0) {
    failed += 1;
    console.error(`FAIL: ${blockingWarnings.length} blocking warning(s)`);
  ...
  if (nodeFiles.length < 200 || nodeFiles.length > 500) {
    failed += 1;
    console.error(
      `FAIL: corpus has ${nodeFiles.length} nodes; envelope is 200-500.`,
    );
  }
  ...
  if (missingFromDisk.length > 0) {
    failed += 1;
    console.error(
      `FAIL: ${missingFromDisk.length} index ID(s) absent from node files. e.g. ${missingFromDisk.slice(0, 3).join(', ')}`,
    );
  }
  ...
  if (missingFromIndex.length > 0) {
    failed += 1;
    console.error(
      `FAIL: ${missingFromIndex.length} node file(s) absent from index. e.g. ${missingFromIndex.slice(0, 3).join(', ')}`,
    );
  }

  if (failed > 0) {
    console.error(`\ndocusaurus-docs conformance: FAILED (${failed} check(s)).`);
    process.exit(1);
  }
  console.log(
    `\ndocusaurus-docs conformance: OK — gaps: 0; declared.level: standard; achieved.level: standard; delivery: static; nodes: ${nodeFiles.length}; subtrees: ${subtreeFiles.length}.`,
  );
}
