...
//     leave additionalProperties open. Decoding into the typed fields plus a
//     separate generic decode is the caller's job; the typed surface here
//     aims at common code paths.
package core
...
// envelopes. Only `summary` is required; `abstract` and `body` are optional.
type Tokens struct {
...
// RelatedRef is one entry in a node's `related` array. Both fields are
// required by the schema; cycles in the related graph are permitted
// (consumer-side cycle detection required).
